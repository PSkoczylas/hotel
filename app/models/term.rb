class Term < ApplicationRecord
  ...
  default_scope -> { order(begin_date: :desc).order(:end_date).order(:id) }
  
  before_validation :set_correct_date
  ...
  protected
    def set_correct_date
      self.begin_date ||= Term.find_by(id: self.id).begin_date if Term.find_by(id: self.id).present?
      self.end_date ||= Term.find_by(id: self.id).end_date if Term.find_by(id: self.id).present?
    end
